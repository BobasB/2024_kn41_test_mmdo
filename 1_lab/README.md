...
* Результати виконання завданнь:
    ...
    1. Окрім написання скриптів на Bash можна також використовувати чисто Пайтонівський шлях для роботи з віртуальними середовищами. Для цього потрібно використовувати спеціальну бібліотеку `pipenv` (зараз набуває популярності схожа утиліта яка називається `poetry`).
        ...
        1. При закритті терміналу, всі змінні середовища пропадають, тому нам кожного разу буде потрібно задавати нашу конфігураційну змінну `PIPENV_VENV_IN_PROJECT`. Для `pipenv` ми можемо всі змінні середовища помісти у фалй [.env](./3_project/.env), вони будуть автоматично присутніми у середовищі, а також ми зможемо такі змінні використовувати у Пайтон програмах. Після перестворення проекту та запуску програми, ми бдемо мати середовище в якому будуть присутні змінні з файла та ми їх зможемо викликати у програмі. Виконуємо команди і дивимось на результат:
            ...
            ```
            ![](./pipenv_env.jpg)
        1. За допомогою ChatGPT потрібно створити проект з веб сайтом написаним на `Flask`. Написавши промт ми отримали наступну відповідь:

            ---

            Ось кроки для створення такого проекту:

            1. **Встановлення pipenv**: Перш за все, переконайтеся, що у вас встановлений pipenv. Якщо він не встановлений, встановіть його,            використовуючи pip:

            ```bash
            pip install pipenv
            ```

            2. **Створення проекту та встановлення Flask**: Створіть нову теку для вашого проекту та перейдіть у неї. Потім використовуйте pipenv,          щоб створити віртуальне середовище та встановити Flask:
            ...
            3. **Створення додатку Flask та сторінок**: Створіть файл `app.py` у вашій текі проекту та додайте наступний код для створення додатку          Flask з двома сторінками:
            ...
            @app.route('/help')
            def help():
                return 'This is the help page. Here you can find information on how to use this website.'

            if __name__ == '__main__':
                app.run(debug=True)
            ```

            4. **Запуск сервера**: Запустіть сервер, щоб перевірити, чи працює ваш веб-сайт:

            ```bash
            pipenv run python app.py
            ```

            Після запуску ви можете перейти за адресою `http://127.0.0.1:5000/` у своєму браузері, щоб побачити головну сторінку з привітанням. Та          за адресою `http://127.0.0.1:5000/help`, щоб побачити сторінку довідки.

            Це базовий приклад використання Flask для створення веб-сайту з двома сторінками. Ви можете розширити цей проект додаванням більш           складної логіки та додаткових сторінок за необхідності.

            ---
        1. Після виконання кроків ми запустили веб сервер та отримали наступний результат ![](./flask.jpg) ![](./flask_web.jpg)

---

### Висновок:
> Висновок офрмляйте самостійно у власних роботах!
